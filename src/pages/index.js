...
export default function Home() {
  ...
  // removed separate title/description states in favor of modal's currentTodo
  ...
  const handleModalSubmit = (e) => {
    ...
    if (isEditing) {
      setTodos(todos.map(t => t.id === currentTodo.id ? { ...currentTodo, date: dateStr } : t));
    } else {
      const newTodo = {
        ...currentTodo,
        id: Date.now(),
    ...
    <div className={`${geistSans.variable} ${geistMono.variable} min-h-screen bg-gray-50 py-8 font-sans`}>
      ...
      {modalOpen && (
        ...
        >
          ...
          >
            <h3 className="text-xl font-semibold mb-4">{isEditing ? "Edit Todo" : "Add New Todo"}</h3>
